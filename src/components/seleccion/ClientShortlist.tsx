...
function extractTokkoId(url: string): string | null {
  // Try numeric ID first (7208329 style)
  const numMatch = /\/(\d{6,8})[-\/]/.exec(url)
  if (numMatch) return numMatch[1]
  // Fallback: any 6-8 digit sequence
  const anyNum = /(\d{6,8})/.exec(url)
  if (anyNum) return anyNum[1]
  return null
}

function getPhoto(d: TokkoData): string | null {
  const photos = (d.photos || []).filter(p => !p.is_blueprint)
  const cover = photos.find(p => p.is_front_cover)
  return (cover || photos[0])?.image || (cover || photos[0])?.thumb || null
}
function getTitle(d: TokkoData): string { return d.publication_title || d.address || d.fake_address || '' }
function getLocation(d: TokkoData): string { return d.location?.short_location || d.location?.name || '' }
function getRooms(d: TokkoData): number { return d.suite_amount || d.room_amount || 0 }
function getArea(d: TokkoData): number { return parseFloat(d.roofed_surface || d.total_surface || d.surface || '0') || 0 }
function getPrice(d: TokkoData): { amount: number; currency: string } | null {
  const p = d.operations?.[0]?.prices?.[0]
  return p?.price ? { amount: p.price, currency: p.currency || 'USD' } : null
}
function fmtPrice(a: number, c: string) { return `${c} ${a.toLocaleString('es-AR')}` }
function isValidNote(note: string | undefined | null): boolean {
  if (!note) return false
  return note.trim().length > 3
}

/* ── SVG Icons (compact 13-14px for chips, 14px for buttons) ── */
const IcBed = () => <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8"><path d="M3 9V19M21 9V19M3 15H21M3 9C3 7.9 3.9 7 5 7H19C20.1 7 21 7.9 21 9"/><path d="M7 7V5C7 4.4 7.4 4 8 4H16C16.6 4 17 4.4 17 5V7"/></svg>
const IcBath = () => <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8"><path d="M4 12H20V19C20 20.1 19.1 21 18 21H6C4.9 21 4 20.1 4 19V12Z"/><path d="M4 12V6C4 4.9 4.9 4 6 4C7.1 4 8 4.9 8 6V8"/><path d="M8 8H20"/></svg>
const IcArea = () => <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8"><rect x="3" y="3" width="18" height="18" rx="2"/><path d="M3 9H21M9 3V21"/></svg>
const IcPrice = () => <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8"><rect x="2" y="6" width="20" height="12" rx="2"/><circle cx="12" cy="12" r="3"/></svg>
const IcHeart = ({ filled }: { filled?: boolean }) => <svg width="14" height="14" viewBox="0 0 24 24" fill={filled ? 'currentColor' : 'none'} stroke="currentColor" strokeWidth="2.5"><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"/></svg>
const IcThumbDown = () => <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="M10 15v4a3 3 0 0 0 3 3l4-9V2H5.72a2 2 0 0 0-2 1.7l-1.38 9a2 2 0 0 0 2 2.3H10z"/><path d="M17 2h2.67A2.31 2.31 0 0 1 22 4v7a2.31 2.31 0 0 1-2.33 2H17"/></svg>
const IcCalendar = () => <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><rect x="3" y="4" width="18" height="18" rx="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg>
const IcPencil = () => <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>
const IcCheck = () => <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><polyline points="20 6 9 17 4 12"/></svg>
  ...
  useEffect(() => {
    ...
    Promise.allSettled(
      toFetch.map(async e => {
        ...
        return { propId: e.id, data: await res.json() }
      })
    ).then(results => {
      setPropData(prev => {
        const next = { ...prev }
        for (const r of results) {
          if (r.status === 'fulfilled') next[r.value.propId] = { loading: false, data: r.value.data }
        }
        for (const e of toFetch) { if (next[e.id]?.loading) next[e.id] = { loading: false, data: null } }
        return next
      })
    })
          ...
          {session.properties.map((prop, idx) => {
            ...
            return (
              <div
                ...
                style={{ boxShadow: '0 2px 12px rgba(0,0,0,0.08)', border: `1px solid ${borderColor}` }}
              >
                {/* Photo */}
                ...
                <div className="p-3.5 flex-1 flex flex-col">
                  ...
                  {location && <p className="text-[12px] text-[#6E6E73] mb-1.5">{location}</p>}

                  {/* Agent note — only if valid */}
                  ...
                  {(rooms > 0 || baths > 0 || area > 0 || price) && (
                    <div className="flex flex-wrap gap-x-2.5 gap-y-1 mb-2.5">
                      {rooms > 0 && <span className="flex items-center gap-1 text-[12px] text-[#6E6E73]"><IcBed /> {rooms} dorm.</span>}
                      {baths > 0 && <span className="flex items-center gap-1 text-[12px] text-[#6E6E73]"><IcBath /> {baths} baño{baths > 1 ? 's' : ''}</span>}
                      {area > 0 && <span className="flex items-center gap-1 text-[12px] text-[#6E6E73]"><IcArea /> {area} m²</span>}
                      {price && <span className="flex items-center gap-1 text-[12px] font-semibold text-[#1A5C38]"><IcPrice /> {fmtPrice(price.amount, price.currency)}</span>}
                    </div>
                  )}
                  ...
                  {/* Buttons */}
                  <div className="pt-3 space-y-1.5" style={{ borderTop: '1px solid #f0f0f0' }}>
                    {/* Like / Dislike row */}
                    <div className="flex gap-1.5">
                      <button
                        onClick={() => patchReaction(prop.id, { liked: r.liked === true ? null : true })}
                        className={`flex-1 flex items-center justify-center gap-1.5 py-2.5 rounded-full text-[13px] font-semibold transition-all active:scale-[0.96] ${
                          r.liked === true ? 'bg-[#1A5C38] text-white' : 'bg-[#F2F2F7] text-[#6E6E73]'
                        }`}
                      >
                        <IcHeart filled={r.liked === true} /> Me gusta
                      </button>
                      <button
                        onClick={() => patchReaction(prop.id, { liked: r.liked === false ? null : false })}
                        className={`flex-1 flex items-center justify-center gap-1.5 py-2.5 rounded-full text-[13px] font-semibold transition-all active:scale-[0.96] ${
                          r.liked === false ? 'bg-[#FF3B30] text-white' : 'bg-[#F2F2F7] text-[#6E6E73]'
                        }`}
                      >
                        <IcThumbDown /> No me gusta
                      </button>
                    </div>
                    ...
                    <button
                      ...
                      className={`w-full flex items-center justify-center gap-1.5 py-2.5 rounded-full text-[13px] font-bold transition-all active:scale-[0.96] ${
                        r.wantVisit ? 'bg-[#1A5C38] text-white' : 'bg-[#1A5C38] text-white opacity-80 hover:opacity-100'
                      }`}
                    >
                      {r.wantVisit ? <IcCheck /> : <IcCalendar />}
                    ...
                    <button
                      ...
                      className={`w-full flex items-center justify-center gap-1.5 py-2 rounded-full text-[13px] font-semibold transition-all ${
                        r.comment ? 'bg-[#FF9500]/10 text-[#FF9500]' : 'bg-[#F2F2F7] text-[#6E6E73]'
                      }`}
                    >
                      <IcPencil /> {r.comment ? 'Ver mi nota' : 'Agregar nota'}
                    </button>
                  </div>
                  ...
                  {editingComment === prop.id && (
                    <div className="mt-2 space-y-2">
                      ...
                      />
                      <div className="flex gap-1.5">
                        <button onClick={() => saveComment(prop.id)} className="flex-1 py-2 bg-[#1A5C38] text-white text-[13px] font-semibold rounded-full">Guardar</button>
                        <button onClick={() => setEditingComment(null)} className="px-3 py-2 bg-[#F2F2F7] text-[#6E6E73] text-[13px] font-semibold rounded-full">Cancelar</button>
                      </div>
                    </div>
                  )}

                  {commentSaved === prop.id && (
                    <p className="mt-1.5 text-[12px] text-[#1A5C38] font-medium flex items-center gap-1"><IcCheck /> Guardado</p>
                  )}
