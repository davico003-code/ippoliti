...
function PropertyCard({ property }: { property: TokkoProperty }) {
  ...
  return (
    <Link
      ...
      className="bg-white rounded-xl overflow-hidden shadow-sm hover:shadow-md transition-all border border-gray-100 group flex flex-col h-full"
    >
      ...
      <div className="p-4 flex-1 flex flex-col">
        ...
        <div className="mt-auto flex items-center justify-between">
          ...
          <div className="flex items-center gap-2 text-xs text-gray-500">
            {area != null && area > 0 && (
              <span className="flex items-center gap-0.5"><Maximize className="w-3 h-3" /><span className="font-numeric">{area}</span>m²</span>
            )}
