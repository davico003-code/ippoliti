      ...
      <div className="max-w-7xl mx-auto px-8 pt-16 pb-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-10 lg:gap-8 pb-12 border-b border-white/15">
          ...
          <div className="lg:col-span-1">
            ...
            <p className="font-lora italic text-white/80 text-[14px] leading-relaxed max-w-[260px]">
              Acompañamos familias en Funes y Roldán desde 1983.
            </p>
            <a href="mailto:info@siinmobiliaria.com" className="mt-6 flex items-center gap-2 text-white/80 hover:text-white transition">
          ...
          {/* Col 2: sedes */}
          <div>
            <p className="font-poppins text-[11px] font-bold tracking-[0.2em] uppercase mb-5" style={{ color: GOLD }}>Sedes</p>
            <div className="space-y-5">
              {SEDES_FOOTER.map(s => (
                <div key={s.nombre}>
                  ...
                  {s.extra && <p className="font-poppins text-white/80 text-[11px] italic mt-0.5">{s.extra}</p>}
                  <p className="font-poppins text-white/75 text-[12px] mt-1">{s.direccion}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          {/* Col 4: redes */}
          <div>
            <p className="font-poppins text-[11px] font-bold tracking-[0.2em] uppercase mb-5" style={{ color: GOLD }}>Seguinos</p>
            <div className="flex items-center gap-3">
              {SOCIAL.map(s => (
                <a key={s.icon} href={s.href} target="_blank" rel="noopener noreferrer" aria-label={s.label}
                  className="w-11 h-11 rounded-full border border-white/30 flex items-center justify-center hover:bg-white hover:text-brand-600 transition">
