...
import { getProperties, type TokkoProperty, getMainPhoto, formatPrice, generatePropertySlug, getCardArea, isMonoambiente } from '@/lib/tokko'
  ...
  return (
    <div className="min-h-screen bg-white">
      <BreadcrumbJsonLd items={[
        ...
        { name: 'Departamentos en el centro de Rosario', url: 'https://siinmobiliaria.com/departamentos-centro-rosario' },
      ]} />
      ...
      <section className="py-12 px-4"><div className="max-w-7xl mx-auto grid md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        {properties.map(p => { const photo = getMainPhoto(p); const r = getCardArea(p); const beds = p.suite_amount || p.room_amount; const mono = isMonoambiente(p); return (
          <Link key={p.id} href={`/propiedades/${generatePropertySlug(p)}`} className="group bg-white rounded-xl overflow-hidden shadow-sm border border-gray-100 hover:shadow-lg transition-all">
            ...
            <div className="p-4"><p className="text-xl font-black text-gray-900 font-numeric mb-1">{formatPrice(p)}</p><p className="text-sm text-gray-600 mb-1">{[(mono ? 'Monoambiente' : beds && `${beds} amb`), r && `${Math.round(r).toLocaleString('es-AR')} m²`].filter(Boolean).join(' · ')}</p><p className="text-sm text-gray-500 truncate">{p.fake_address || p.address}</p></div>
          </Link>
        )})}
      ...
      <section className="py-12 px-4 bg-[#1A5C38] text-center text-white">
        ...
        <a href="https://wa.me/5493413340916?text=Hola!%20Busco%20departamento%20en%20Centro%20Rosario" target="_blank" rel="noopener noreferrer" className="px-6 py-3 bg-[#25D366] text-white font-bold rounded-xl inline-block">WhatsApp</a>
      </section>
