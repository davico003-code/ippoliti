...
import { getProperties, type TokkoProperty, getMainPhoto, formatPrice, generatePropertySlug, getCardArea } from '@/lib/tokko'
  ...
  return (
    <div className="min-h-screen bg-white">
      <BreadcrumbJsonLd items={[
        ...
        { name: 'Casas en venta en Fisherton', url: 'https://siinmobiliaria.com/casas-en-venta-fisherton' },
      ]} />
      ...
      <section className="py-12 px-4">
        <div className="max-w-7xl mx-auto grid md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {properties.map(p => { const photo = getMainPhoto(p); const area = getCardArea(p); const beds = p.suite_amount || p.room_amount; return (
            <Link key={p.id} href={`/propiedades/${generatePropertySlug(p)}`} className="group bg-white rounded-xl overflow-hidden shadow-sm border border-gray-100 hover:shadow-lg transition-all">
              ...
              <div className="p-4">
                ...
                <p className="text-sm text-gray-600 mb-1">{[beds && `${beds} dorm`, area && `${Math.round(area).toLocaleString('es-AR')} m²`].filter(Boolean).join(' · ')}</p>
      ...
      <section className="py-12 px-4 bg-[#1A5C38] text-center text-white">
        ...
        <a href="https://wa.me/5493413340916?text=Hola!%20Busco%20casa%20en%20Fisherton" target="_blank" rel="noopener noreferrer" className="px-6 py-3 bg-[#25D366] text-white font-bold rounded-xl inline-block">WhatsApp</a>
      </section>
