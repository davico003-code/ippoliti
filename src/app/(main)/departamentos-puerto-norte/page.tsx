    ...
    <div className="min-h-screen bg-white">
      ...
      <section className="py-12 px-4">
        <div className="max-w-7xl mx-auto grid md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {properties.map(p => { const photo = getMainPhoto(p); const area = getCardArea(p); const beds = p.suite_amount || p.room_amount; const mono = isMonoambiente(p); return (
            <Link key={p.id} href={`/propiedades/${generatePropertySlug(p)}`} className="group bg-white rounded-xl overflow-hidden shadow-sm border border-gray-100 hover:shadow-lg transition-all">
              ...
              <div className="p-4">
                ...
                <p className="text-sm text-gray-600 mb-1">{[(mono ? 'Monoambiente' : beds && `${beds} dorm`), area && `${Math.round(area).toLocaleString('es-AR')} m²`].filter(Boolean).join(' · ')}</p>
